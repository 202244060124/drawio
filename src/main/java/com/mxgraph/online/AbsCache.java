...
abstract public class AbsCache extends HttpServlet implements AbsComm {
     ...
     */
protected
    static final String PUSHER_CONFIG_FILE_PATH = "pusher_properties";
...
protected
     ...
     */
protected
    static final int expirationDelta = 300;
...
protected
    ...
    static String createCacheEntryKey(String id, String version)
    {
        return id + ":" + version;
    }
...
protected
    ...
    static String createLastVersionKey(String id, String version)
    {
        return id + "@" + version;
    }
...
protected
    ...
    static String createTokenKey(String id, String secret)
    {
        return id + "#" + secret;
    }
...
protected
    ...
    Pusher getPusher() throws IOException
    {
        ...
        if (pusher == null) {
            ...
            pusher = new Pusher(prop.getProperty("app_id"), prop.getProperty("key"), prop.getProperty("secret"));
...
protected
    ...
    void doGetAbst(Object request, Object response) throws IOException
    {
        String respBody = "";
        ...
        try {
            ...
            String domain = ref.toLowerCase().matches("^https?://([a-z0-9,-]+[.])*draw[.]io/.*") ? ".draw.io/" : null;
            domain = (domain == null) ? (ref.toLowerCase().matches("^https?://([a-z0-9,-]+[.])*diagrams[.]net/.*") ? ".diagrams.net/" : null)
                                      : domain;
            ...
            if (stats || alive || domain != null) {
                if (stats) {
                    ...
                    setHeader("Access-Control-Allow-Origin", ref.toLowerCase().substring(0, ref.indexOf(domain) + domain.length() - 1),
                              response);
                    ...
                    if (alive) {
                        ...
                        if (id != null) {
                            try {
                                if (secret != null && (from == null || to == null)) {
...
protected
    ...
    String createToken(String id, String secret) throws UnauthorizedException
    {
...
protected
    ...
    void checkPatch(String id, String current, String secret)
    {
        ...
        if (lastVersion != null) {
            ...
            if (entry != null) {
                if (entry.getSecret() == null || !entry.getSecret().equals(secret)) {
                    cache.remove(key);
                    debug("patch removed id=" + id + " from=" + lastVersion + " to=" + current);

                    // Marks the chain as incomplete
                    cache.put(key, new CacheEntry(null, null, null));
                } else {
                    debug("patch checked id=" + id + " from=" + lastVersion + " to=" + current);
                }
            }
        } else {
...
protected
    ...
    String getPatches(String id, String from, String to, String secret) throws UnauthorizedException, IncompleteChainException
    {
        ...
        while (!seen.contains(current)) {
            ...
            if (entry != null) {
                if (entry.getData() == null || entry.getNext() == null || entry.getSecret() == null) {
        ...
        debug("getPatches id=" + id + " from=" + from + " to=" + to + " data=" + data);
...
protected
    ...
    void doPostAbst(Object request, Object response) throws IOException
    {
        try {
            String ref = getHeader("referer", request);
            // Set ref to something to avoid extra null branch for sc_forbidden
            ...
            if (domain != null) {
                ...
                if (id != null) {
                    ...
                    addPatch(id, getPostParameter("data", request), getPostParameter("secret", request), getPostParameter("token", request),
                             getPostParameter("from", request), getPostParameter("to", request), getPostParameter("last-secret", request));
                    sendMessage(id, getPostParameter("msg", request), getPostParameter("sid", request));
...
protected
    ...
    void addPatch(String id, String data, String secret, String token, String from, String to, String lastSecret) throws UnauthorizedException
    {
        if (secret != null && cache.remove(createTokenKey(id, secret), token)) {
            if (from != null && to != null && data != null && data.length() < maxCacheSize) {
                ...
                debug("addPatch id=" + id + " from=" + from + " to=" + to + " secret=" + secret + " token=" + token + " data=" + data);
            }
        } else if (data != null) {
            throw new UnauthorizedException();
        }
...
protected
    ...
    void sendMessage(String id, String msg, String sid) throws IOException
    {
        if (msg != null) {
...
public
    ...
    public
        ...
        CacheEntry(String next, String data, String secret)
        {
            this.next = next;
            this.data = data;
            this.secret = secret;
