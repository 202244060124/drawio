...
DrawioViewer.prototype.init = function()
{
    ...
    this.graph.click = function(me)
    {
        ...
        if (cell != null && !me.isConsumed() && (mxEvent.isTouchEvent(me.getEvent()) || mxEvent.isLeftMouseButton(me.getEvent()))) {
            ...
            if (href != null) {
                var r = new RegExp("^(?:[a-z]+:)?//",
                                   "i"); // https://stackoverflow.com/questions/10687099/how-to-test-if-a-url-string-is-absolute-or-relative

                if (!r.test(href)) {
                    // relative link

                    if (href.lastIndexOf("/", 0) != = 0) {
                        // http://stackoverflow.com/a/4579228/226469 seems to be the fastest check
                    ...
                    // window.location.href = href.substring(0, cp.length) === cp ? href : cp + href; // prepends the context path if it's
                    // not already there
    ...
    graph.lazyZoom = function(zoomIn)
    {
        ...
        this.updateZoomTimeout = window.setTimeout(mxUtils.bind(
                                                       this,
                                                       function() {
    ...
    if (this.lightbox) {
        mxEvent.addMouseWheelListener(mxUtils.bind(
            this, function(evt, up) {
    ...
    if (!this.lightbox) {
        var update = mxUtils.bind(
            this, function() {
                ...
                this.graph.container.style.backgroundColor =
                    this.graph.background == null || this.graph.background == "none" ? "#ffffff" : this.graph.background;
            });
...
DrawioViewer.prototype.addToolbarButton = function(toolbar, drawioButton)
{
    ...
    if (!drawioButton.enabled) {
        ...
        if (drawioButton.linkButton) {
            ...
            mxEvent.addListener(button, "click", function(evt) { drawioButton.clickHandler.apply(this, arguments); });
        }
        mxEvent.addListener(button, "mouseover", function(evt) { button.className += " diagramly-reader-toolbar-button-hover"; });
        mxEvent.addListener(button, "mouseout", function(evt) { button.className = "diagramly-reader-toolbar-button"; });
    }
...
, enabled : canRemove,
    ...
    buttons[DrawioViewerActions.EXPAND] = new DrawioViewerButton({
        ...
        clickHandler : mxUtils.bind(
            this, function() { this.showLightbox(); }),
        enabled : enableButton,
    });
    ...
    , enabled : enableButton, visible : false,
    });
