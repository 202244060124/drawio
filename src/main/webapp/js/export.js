...
if (mxIsElectron) {
    mxmeta("default-src 'self'; script-src 'self'; connect-src 'self' https://*.draw.io https://*.diagrams.net https://fonts.googleapis.com https://fonts.gstatic.com; img-src * data:; media-src *; font-src *; frame-src 'none'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; base-uri 'none';child-src 'self';object-src 'none';",
           "Content-Security-Policy");

    // We can't use eval in Electron because of CSP, so load all shapes and disable eval
    mxscript(
        "js/stencils.min.js", function() {
            mxscript(
                "js/shapes-14-6-5.min.js", function() {
...
function render(data)
{
            ...
            document.fonts.ready.then(function() {
                ...
                if (mxIsElectron) {
                    try {
                        ...
                        // For some reason, Electron 9 doesn't send this object as is without stringifying. Usually when variable is
                        // external to function own scope
                        electron.sendMessage("render-finished", {
    ...
    function waitForFont(url)
    {
        try {
            if (url != null && fontPreload[url] == null) {
                ...
                mxUtils.get(url, decrementWaitCounter, decrementWaitCounter, false, 10000, decrementWaitCounter);
            }
        } catch (e) {
            // ignore font
        }
    ...
    Graph.createFontElement = function(name, url)
    {
        ...
        try {
            if (elt != null && elt.nodeName.toLowerCase() == "link" && elt.getAttribute("type") == "text/css"
    ...
    function renderPage(currentPageId)
    {
        ...
        if (bgImg != null) {
            ...
            graph.setBackgroundImage(new mxImage(bgImg.src, bgImg.width, bgImg.height, bgImg.x, bgImg.y));
        } else {
        ...
        // handle layers
        if (extras != null
            && ((extras.layers != null && extras.layers.length > 0) || (extras.layerIds != null && extras.layerIds.length > 0))) {
        ...
        if (data.print || data.format == "pdf") {
            ...
            if (data.fit == "1" && data.sheetsAcross != null && data.sheetsDown != null) {
                ...
                scale = Math.min((pf.height * v) / (gb.height / graph.view.scale), (pf.width * h) / (gb.width / graph.view.scale));
            } else {
            ...
            graph.view.getBackgroundPageBounds = function()
            {
                ...
                return new mxRectangle(this.scale * (this.translate.x + layout.x * page.width),
                                       this.scale * (this.translate.y + layout.y * page.height), this.scale * layout.width * page.width,
        ...
        if (!graph.pdfPageVisible) {
            ...
            if (data.w > 0 || data.h > 0) {
                ...
                graph.view.scaleAndTranslate(s, Math.floor(data.border / s - Math.floor(b.x)),
                ...
                graph.view.scaleAndTranslate(s, Math.floor(data.border - Math.floor(b.x)), Math.floor(data.border - Math.floor(b.y)));
            }
        } else {
        ...
        bounds.width =
            Math.ceil(bounds.width + data.border) + 1;  // The 1 extra pixels to prevent cutting the cells on the edges when crop is enabled
        bounds.height =
            Math.ceil(bounds.height + data.border) + 1; // The 1 extra pixels to prevent starting a new page. TODO Not working in every case
        ...
        if (graph.pdfPageVisible) {
            ...
            var autoOrigin = ((data.print || data.format == "pdf") && data.fit == "1") || data.crop == "1"
            ...
            if (preview == null) {
                ...
                preview.pageMargin = data.pageMargin != null ? parseInt(data.pageMargin) : data.crop == "1" ? 0 : preview.pageMargin;
                ...
                preview.drawBackgroundImage = function(img)
                {
                    ...
                    if (img.node != null) {
                        ...
                        graph.disableSvgLinks(img.node, function(link) { link.setAttribute("href", "javascript:void(0)"); });
                    }
                };
            ...
            if (bgImg != null) {
                ...
                if (t.x < 0 || t.y < 0) {
                    graph.view.setTranslate(t.x < 0 ? Math.max(-bgImg.x * s, t.x) : t.x, t.y < 0 ? Math.max(-bgImg.y * s, t.y) : t.y);
                    bounds.x = 0.5;
                    bounds.y = 0.5;
                }
            }
    ...
    if (diagrams != null && diagrams.length > 0) {
        ...
        {
            if (name == "page") {
                return diagrams == null ? "Page-1" : diagrams[from].getAttribute("name") || "Page-" + (from + 1);
    ...
    if (fallbackFont) {
        ...
        document.querySelectorAll("foreignObject div")
            .forEach((d) = > (d.style.fontFamily = (d.style.fontFamily || "") + ", " + fallbackFont));
    }
