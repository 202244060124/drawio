(function(stylesheet, stencils)
{
    ...
    try {
        var style = document.createElement('style') style.type = 'text/css' style.innerHTML =
            [
                'div.mxTooltip {', '-webkit-box-shadow: 3px 3px 12px #C0C0C0;', '-moz-box-shadow: 3px 3px 12px #C0C0C0;',
                'box-shadow: 3px 3px 12px #C0C0C0;', 'background: #FFFFCC;', 'border-style: solid;', 'border-width: 1px;',
                'border-color: black;', 'font-family: Arial;', 'font-size: 8pt;', 'position: absolute;', 'cursor: default;',
    ...
    function renderMath(nodes)
    {
        try {
            ...
            if (e.retry != null) {
                e.retry.then(function() { MathJax.typesetPromise(nodes); });
    ...
    function loadMathJax()
    {
        ...
        if (!mathJaxLoading) {
            ...
            window.MathJax = {
                options : {skipHtmlTags : {'[+]' : ['text']}},
                loader : {
                    load :
                        [ (urlParams['math-output'] == 'html') ? 'output/chtml' : 'output/svg', 'input/tex', 'input/asciimath', 'ui/safe' ]
                },
                startup : {pageReady : function(){renderMath(mathJaxQueue);
        }
    }
            };
    ...
    function initGraph(container)
    {
        ...
        if (url != null) {
            try {
                ...
                var xhr = (navigator.userAgent != null && navigator.userAgent.indexOf('MSIE 9') > 0) ? new XDomainRequest()
                                                                                                     : new XMLHttpRequest();
                xhr.open('GET', url);

                xhr.onload = mxUtils.bind(
                    this, function() {
                        try {
        ...
        function addButton(label, funct)
        {
            ...
            mxEvent.addListener(btn, (mxClient.IS_POINTER) ? 'pointerdown' : 'mousedown', function(evt) { mxEvent.consume(evt); });

            mxEvent.addListener(
                btn, (mxClient.IS_POINTER) ? 'pointerup' : 'mouseup', function(evt) {
                    funct();
                    mxEvent.consume(evt);
                });

            if (!mxClient.IS_POINTER && mxClient.IS_TOUCH) {
                mxEvent.addListener(btn, 'touchstart', function(evt) { mxEvent.consume(evt); });

                mxEvent.addListener(
                    btn, 'touchend', function(evt) {
                        funct();
                        mxEvent.consume(evt);
                    });
            }
