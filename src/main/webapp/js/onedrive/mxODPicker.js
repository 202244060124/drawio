function mxODPicker(container, previewFn, getODFilesList, getODFileInfo, getRecentList, addToRecent, pickedFileCallback, errorFn,
                    foldersOnly, backFn, withSubmitBtn, withThumbnail, initFolderPath, acceptAllFiles)
{
    var previewHtml = "";
    ...
    var html = '<div class="odCatsList">' + '<div class="odCatsListLbl">OneDrive</div>'
               ...
               + "</div>" + '<div class="odFilesSec">'
               + '<div class="searchBar" style="display:none"><input type="search" id="odSearchBox" placeholder="'
               + mxUtils.htmlEntities(mxResources.get("search")) + '"></div>' + '<div class="odFilesBreadcrumb"></div>'
               + '<div id="refreshOD" class="odRefreshButton">'
               + '<img class="geAdaptiveAsset" src="/images/update32.png" width="16" height="16" title="'
               ...
               + (withSubmitBtn ? '<button id="odSubmitBtn" class="odSubmitBtn">'
                                      + mxUtils.htmlEntities(mxResources.get(foldersOnly ? "select" : "open")) + "</button>"
                                : "");

    var isDarkMode = window.Editor != null && Editor.isDarkMode != null && Editor.isDarkMode();

    var css =
        ".odCatsList *, .odFilesSec * { user-select: none; }" + ".odCatsList {" + "    box-sizing: border-box;" + "    position:absolute;"
        + "    top:0px;" + "    bottom:50%;" + "    width:30%;" + "    border: 1px solid #CCCCCC;" + "    border-bottom:none;"
        + "    display: inline-block;" + "    overflow-x: hidden;" + "    overflow-y: auto;" + "}" + ".odCatsListLbl {"
        + "    height: 17px;" + "    color: #6D6D6D;" + "    font-size: 14px;" + "    font-weight: bold;" + "    line-height: 17px;"
        + "    margin: 10px 0 3px 5px;" + "}" + ".odFilesSec {" + "    box-sizing: border-box;" + "    position:absolute;" + "    left:30%;"
        + "    top:0px;" + "    bottom:50%;" + "    width: 70%;" + "    border: 1px solid #CCCCCC;" + "    border-left:none;"
        + "    border-bottom:none;" + "    display: inline-block;" + "    overflow: hidden;" + "}" + ".odFilesBreadcrumb {"
        + "    box-sizing: border-box;" + "    position:absolute;" + "    min-height: 32px;" + "    left:0px;" + "    right:20px;"
        + "    text-overflow:ellipsis;" + "    overflow:hidden;" + "    font-size: 13px;" + "    color: #6D6D6D;" + "    padding: 5px;"
        + "}" + ".odRefreshButton {" + "    box-sizing: border-box;" + "    position:absolute;" + "    right:0px;" + "    top:0px;"
        + "    padding: 4px;" + "    margin: 1px;" + "    height:24px;" + "    cursor:default;" + "}" + ".odRefreshButton>img {"
        + "    opacity:0.5;" + "}" + ".odRefreshButton:hover {" + "    background-color:#ddd;" + "    border-radius:50%;" + "}" +
        // '.odRefreshButton:hover {' +
        // '    background-color:#ddd;' +
        // '    border-radius:50%;' +
        // '}' +
        ".odRefreshButton:active {" + "    opacity:0.7;" + "}" + ".odFilesList {" + "    box-sizing: border-box;" + "    position:absolute;"
        + "    top:32px;" + "    bottom:0px;" + "    width: 100%;" + "    overflow-x: hidden;" + "    overflow-y: auto;" + "}"
        + ".odFileImg {" + "    width: 24px;" + "    padding-left: 5px;" + "    padding-right: 5px;" + "}" + ".odFileTitle {"
        + "    cursor: default;" + "    font-weight: normal;" + "    color: #666666 !important;" + "    width: calc(100% - 20px);"
        + "    white-space: nowrap;" + "    overflow: hidden;" + "    text-overflow: ellipsis;" + "}" + ".odFileListGrid {"
        + "    width: 100%;" + "    white-space: nowrap;" + "    font-size: 13px;" + "    box-sizing: border-box;"
        + "    border-spacing: 0;" + "}" + ".odOddRow {" + (isDarkMode ? "" : "    background-color: #eeeeee;") + "}" + ".odEvenRow {"
        + (isDarkMode ? "" : "    background-color: #FFFFFF;") + "}" + ".odRowSelected {" + "    background-color: #cadfff;" + "}"
        + ".odCatListTitle {" + "    box-sizing: border-box;" + "    height: 17px;" + "    cursor: default;" + "    color: #666666;"
        + "    font-size: 14px;" + "    line-height: 17px;" + "    margin: 5px 0 5px 0px;" + "    padding-left: 10px;" + "}"
        + ".odCatSelected {" + "    font-weight: bold;" + "    background-color: #cadfff;" + "}" + ".odEmptyFolder {" + "    height: 17px;"
        + "    color: #6D6D6D;" + "    font-size: 14px;" + "    font-weight: bold;" + "    line-height: 17px;"
        + "    margin: 10px 0 3px 5px;" + "    width: 100%;" + "    text-align: center;" + "}" + ".odBCFolder {" + "    cursor: pointer;"
        + "    color: #0432ff;" + "}" + ".odPreviewStatus {" + "    position:absolute;" + "    text-align:center;" + "    width:100%;"
        + "    top:50%;" + "    transform: translateY(-50%);" + "    font-size:13px;" + "    opacity:0.5;" + "}" + ".odPreview {"
        + "    position:absolute;" + "     overflow:hidden;" + "     border: 1px solid #CCCCCC;" + "    bottom:0px;" + "    top: 50%;"
        + "    left:0px;" + "    right:0px;" + "}" + ".odLinkBtn {" + "   position: absolute;" + "    font-size: 12px;"
        + "    cursor: pointer;" + "    color: #6D6D6D;" + "    left: 5px;" + "    bottom: 3px;" + "}" + ".odSubmitBtn {"
        + "   position: absolute;" + "    color: #333;" + "    right: 5px;" + "    bottom: 5px;" + "}";
    ...
    function getDrawioFileDoc(file, success, error)
    {
        if (file["@microsoft.graph.downloadUrl"] == null) {
            if (file.parentReference == null) {
                ...
                getODFileInfo(
                    file.id, file.parentReference.driveId, function(completeFile) { getDrawioFileDoc(completeFile, success, error); },
                    error);
            ...
            if (this.readyState == 4) {
                if (this.status >= 200 && this.status <= 299) {
                    try {
                        ...
                        var node = doc.documentElement.nodeName == "mxlibrary" ? doc.documentElement
                                                                               : Editor.extractGraphModel(doc.documentElement);
                    ...
                    } // on error and if the doc is null, the following line will call the error
                }
    ...
    function doSubmit()
    {
        ...
        if (withThumbnail && curViewer != null) {
            editor.exportToCanvas(
                function(canvas) { submit(EditorUi.prototype.createImageDataUri(canvas, null, "png")); }, 400, null, null,
                function(err) {
                    // TODO handle errors
                    console.log(err);
                },
    ...
    function openFile()
    {
        if (selectedFile == null || requestInProgress)
            return;
        ...
        if (selectedDriveId == "sharepoint") {
            ...
            selectedFile = selectedFile.remoteItem ? selectedFile.remoteItem
                                                   : selectedFile; // handle remote items which is accessed indirectly
    ...
    function fillFolderFiles(driveId, folderId, siteId, folderName, searchTxt)
    {
        ...
        function renderList(potentialDrawioFiles)
        {
            ...
            for (var i = 0; potentialDrawioFiles != null && i < potentialDrawioFiles.length; i++) {
                ...
                if (isSharepointSites == 1 && item.webUrl
                    && !(item.webUrl.indexOf("sharepoint.com/sites/") > 0 || item.webUrl.indexOf("sharepoint.com/") < 0)) {
                    continue;
                }
        ...
        switch (driveId) {
            ...
            case "search": // TODO search doesn't return any results, find out why then remove display: none from the searchBox
                ...
                url = selectedSiteId
                    ? "/sites/" + selectedSiteId + "/drive/root/search(q='" + searchTxt + "')"
                    : driveId
                    ? "/drives/" + driveId + "/root/search(q='" + searchTxt + "')"
                    : "/me/drive/root/search(q='" + searchTxt + "')";
                break;
            default:
                if (folderId == null) {
        ...
        if (!isSharepointSites) {
            url +=
                (url.indexOf("?") > 0 ? "&" : "?")
                + "select=id,name,description,parentReference,file,createdBy,lastModifiedBy,lastModifiedDateTime,size,folder,remoteItem,@microsoft.graph.downloadUrl";
        }
                ...
                function(resp) {
                    ...
                    if (acceptAllFiles || isSharepointSites) {
                        ...
                        for (var i = 0; i < list.length; i++) {
                            ...
                            if (file.folder || mimeType == "text/html" || mimeType == "text/xml" || mimeType == "application/xml"
                                || mimeType == "image/png" || /\.svg$ /.test(file.name) || /\.html$ /.test(file.name) ||
                                /\.xml$ /.test(file.name) || /\.png$ /.test(file.name) || /\.drawio$ /.test(file.name) ||
                                /\.drawiolib$ /.test(file.name) || /\.pdf$ /.test(file.name)) {
                    ...
                    if (resp["@odata.nextLink"] && potentialDrawioFiles.length < 1000) {
                        // TODO Support dynamic paging instead of 1000 limit
        ...
        .addEventListener(
            "keyup", function(evt) {
                ...
                if (evt.keyCode == 13) {
                    ...
                    delayTimer = setTimeout(function() { doSearch(searchInput.value); }, 500);
                }
            });
