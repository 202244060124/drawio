...
(async function() {
    let requestSync = async function(msg)
    {
        ...
        async function doRequest()
        {
            return new Promise((resolve, reject) = > {
                electron.request(msg, function(data) { resolve(data); }, function(errMsg, e) { reject(e || errMsg); });
            });
        }
    ...
    PrintDialog.electronPrint = function(editorUi, args)
    {
        ...
        var xml = editorUi.getFileData(true, null, null, null, !args.selection, false, null, null, null, false, true);
    ...
    Menus.prototype.init = function()
    {
        ...
        editorUi.actions.put("useOffline",
                             new Action(mxResources.get("useOffline") + "...", function() { editorUi.openLink("https://www.draw.io/"); }));
        ...
        this.put("openRecent", new Menu(function(menu, parent) {
                     ...
                     menu.addItem(mxResources.get("reset"), null, function() { editorUi.resetRecent(); }, parent);
                 }));

        // Replaces file menu to replace openFrom menu with open and rename downloadAs to export
        this.put("file", new Menu(mxUtils.bind(
                             this, function(menu, parent) {
    ...
    EditorUi.prototype.init = async function()
    {
        ...
        this.actions.addAction("newLibrary...",
                               mxUtils.bind(this, function() { editorUi.showLibraryDialog(null, null, null, null, App.MODE_DEVICE); }));

        this.actions.addAction("openLibrary...", mxUtils.bind(this, function() { editorUi.pickLibrary(App.MODE_DEVICE); }));

        // Replaces import action
        this.actions.addAction("import...",
                               mxUtils.bind(
                                   this, async function() {
                                       if (editorUi.getCurrentFile() != null) {
                                           ...
                                           if (paths != = undefined && paths[0] != null) {
                                               ...
                                               var encoding = asImage || /\.pdf$ / i.test(path) || /\.vsdx$ / i.test(path) ||
                                                                      /\.vssx$ / i.test(path)
                                                                  ? "base64"
                                                                  : "utf-8";
                                            ...
                                            editorUi.importVisio(
                                                editorUi.base64ToBlob(data, "application/octet-stream"), function(xml) {
                                                    editorUi.spinner.stop();
                                                    graph.setSelectionCells(editorUi.importXml(xml));
                                                });
        ...
        fullscreenAction.setSelectedCallback(function() { return isFullScreen; });
        ...
        editorUi.actions.addAction(
            "plugins...", function() {
                ...
                editorUi.showDialog(new PluginsDialog(
                                        editorUi,
                                        async function(callback) {
                                            ...
                                            for (var i = 0; i < App.publicPlugin.length; i++) {
                                                ...
                                                if (pluginsMap[App.pluginRegistry[p]])
                                                    continue;
                                            ...
                                            if (await requestSync("isPluginsEnabled")) {
                                                var extPluginsBtn = mxUtils.button(
                                                    mxResources.get("selectFile") + "...", async function() {
                                            ...
                                            var dlg = new CustomDialog(editorUi, div,
                                                                       mxUtils.bind(
                                                                           this, function() {
    ...
    App.prototype.load = function()
    {
        ...
        electron.registerMsgListener("args-obj", (argsObj) = > { this.loadArgs(argsObj); });
    ...
    App.prototype.loadArgs = function(argsObj)
    {
        ...
        if (paths != = undefined && paths[0] != null && paths[0].indexOf("-") != 0
                       && this.spinner.spin(document.body, mxResources.get("loading"))) {
            var path = paths[0];
            this.hideDialog();

            var success = mxUtils.bind(
                this, function(fileEntry, data, stat, name, isModified) {
                    this.spinner.stop();
        ...
        if (file != null) {
            ...
            if (file.fileObject != null) {
                ...
                await requestSync({
                    ...
                    listener : mxUtils.bind(
                        this,
                        function(curr, prev) {
                            EditorUi.debug("EditorUi.watchFile", [this], "file", [file], "stat", [file.stat], "curr", [curr],
                                           "prev", [prev], "inConflictState", file.inConflictState, "unwatchedSaves", file.unwatchedSaves);
                                ...
                                file.addConflictStatus(
                                    ...
                                    mxUtils.bind(
                                        this, function() {
                                            ...
                                            file.synchronizeFile(mxUtils.bind(
                                                                     this, function() { file.handleFileSuccess(false); }),
                                                                 mxUtils.bind(this, function(err) { file.handleFileError(err, true); }));
                                        }));
                            }
                        }),
                });
    ...
    {
        var doPickFile = mxUtils.bind(
            this, function() {
                this.chooseFileEntry(mxUtils.bind(
                    this, function(fileEntry, data, stat, name, isModified) {
    ...
    {
        this.chooseFileEntry(mxUtils.bind(
            this, function(fileEntry, data, stat) {
                try {
    ...
    App.prototype.chooseFileEntry = async function(fn)
    {
        ...
        if (paths != = undefined && paths[0] != null) {
            ...
            this.readGraphFile(fn, mxUtils.bind(this, function(err) { this.handleError(err); }), paths[0]);
        } else {
            this.spinner.stop();
        }
    };
    ...
    EditorUi.prototype.normalizeFilename = function(title, defaultExtension)
    {
        ...
        if (tokens.length == 1 || mxUtils.indexOf([ "xml", "html", "drawio", "png", "svg" ], ext) < 0) {
                ...
                electron.request(
                    ...
                    mxUtils.bind(
                        ...
                        function(stat_p) {
                            ...
                            electron.request({action : "isFileWritable", file : path},
                                             mxUtils.bind(
                                                 this, function(isWritable) {
                                                     if (!isWritable) {
                                                         var file = this.getCurrentFile();
    ...
    LocalFile.prototype.getLatestVersion = function(success, error)
    {
        if (this.fileObject == null) {
            ...
            this.ui.readGraphFile(mxUtils.bind(
                                      this,
    ...
    LocalFile.prototype.saveFile = async function(revision, success, error, unloading, overwrite)
    {
        ...
        if (typeof success != = "function") {
            if (typeof unloading == = "function") {
                ...
                }); // Original draw.io function parameters are (title, revision, success, error, useCurrentData)
            }
            return;
        }
        ...
        if (!this.savingFile) {
            var fn = mxUtils.bind(
                this, function() {
                    var doSave = mxUtils.bind(
                        this, function(data, enc) {
                            ...
                            this.unwatchedSaves =
                                true; // Multiple saves doesn't call watch the same number, so use a boolean and check for changes
                    ...
                    if (! / (\.png)$ / i.test(this.fileObject.name)) {
                        ...
                        this.ui.getEmbeddedPng(function(data) { doSave(atob(data), "binary"); }, error, null, p.scale, p.border);
                    }
                });
            ...
            try {
                if (this.fileObject == null) {
                    ...
                    var name = this.ui.normalizeFilename(this.getTitle(), this.constructor == LocalLibrary ? "xml" : null);
                    var ext = null;
        ...
        if (this.fileObject == null) {
            ...
            electron.request({
                ...
                data : data != null ? data : this.ui.getFileData(),
                             ...
                             mxUtils.bind(
                                 this, function(draftFileName) { this.fileObject.draftFileName = draftFileName; }),
    ...
    EditorUi.prototype.createDownloadRequest = function(filename, format, ignoreSelection, base64, transparent, currentPage, scale, border,
                                                        grid, includeXml, pageRange, w, h, crop, margin, fit, sheetsAcross, sheetsDown)
    {
        var params = this.downloadRequestBuilder(filename, format, ignoreSelection, base64, transparent, currentPage, scale, border, grid,
                                                 includeXml, pageRange, w, h, crop, margin, fit, sheetsAcross, sheetsDown);
    ...
    ExportDialog.exportFile = function(editorUi, name, format, bg, s, b, dpi)
    {
        ...
        if (format == "xml" || format == "svg") {
            ...
            if ((format == "png" || format == "jpg" || format == "jpeg") && editorUi.isExportToCanvas()) {
                ...
                editorUi.saveRequest(
                    name, format, function(newTitle, base64) {
    ...
    EditorUi.prototype.saveData = async function(filename, format, data, mimeType, base64Encoded)
    {
        ...
        window.setTimeout(mxUtils.bind(
                              this,
                              async function() {
                                  var dlgConfig = {
                                      action : "showSaveDialog",
                                      ...
                                      if (data == null || data.length == 0) {
                                          ...
                                          electron.request({
                                              ...
                                              enc : base64Encoded ? "base64" : "utf-8",
                                                           ...
                                                           mxUtils.bind(
                                                               this, function() { this.spinner.stop(); }),
                                                           mxUtils.bind(
                                                               this, function(e) {
                                                                   this.spinner.stop();
    ...
    {
        this.readGraphFile(mxUtils.bind(
                               this,
                               function(fileEntry, data, stat) {
                                   var library = new DesktopLibrary(this, data, fileEntry);
                                   this.loadLibrary(library);
                                   this.showSidebar();
                                   success(library);
