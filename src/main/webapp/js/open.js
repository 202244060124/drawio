...
function main()
{
    if (window.parent != null && window.parent.Editor != null) {
        ...
        var darkMode = window.parent.Editor.cssDarkMode
                       || (typeof window.parent.Editor.isDarkMode == = "function" && window.parent.Editor.isDarkMode());
        ...
        window.parent.listBrowserFiles(function(filesInfo) {
            if (window.parent != null) {
                if (filesInfo.length == 0) {
                    ...
                    filesInfo.sort(function(a, b) { return a.title.toLowerCase().localeCompare(b.title.toLowerCase()); });
                    ...
                    for (var i = 0; i < filesInfo.length; i++) {
                        ...
                        if (fileInfo.title.length > 0) {
                            ...
                            window.parent.mxEvent
                                .addListener(img, "click", (function(k) {
                                                 return function()
                                                 {
                                                     if (window.parent.mxUtils.confirm(window.parent.mxResources.get("delete") + ' "' + k
                                                                                       + '"?')) {
                                                         window.parent.deleteBrowserFile(k, function() { window.location.reload(); });
                                                     }
                                                 };
                                             })(fileInfo.title));
                            ...
                            window.parent.mxEvent.addListener(link, "click", (function(k) {
                                                                  return function()
                                                                  {
                                                                      if (window.parent.openNew && window.parent.baseUrl != null) {
                                                                          ...
                                                                          window.parent.openBrowserFile(
                                                                              k,
                                                                              function(data) {
                                                                                  if (window.parent != null) {
                                                                                      window.parent.geOpenWindow(
                                                                                          window.parent.baseUrl + "#L"
                                                                                              + encodeURIComponent(k),
                                                                                          function() { of.cancel(false); },
                                                                                          function() { of.setData(data, k); });
                                                                                  }
                                                                              },
                                                                              function(){
                                                                                  // TODO add error
                ...
                var closeButton = window.parent.mxUtils.button(window.parent.mxResources.get("close"), function() { hideWindow(true); });
