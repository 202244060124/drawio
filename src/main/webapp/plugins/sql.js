...
Draw.loadPlugin(function(ui) {
    ...
    function AddRow(propertyModel, tableName)
    {
        ...
        if (propertyModel.IsForeignKey&& propertyModel.ForeignKey != = undefined&& propertyModel.ForeignKey != = null) {
            propertyModel.ForeignKey.forEach(function(foreignKeyModel) {
                ...
                if (tableName != =
                        foreignKeyModel.PrimaryKeyTableName
                        || (tableName === foreignKeyModel.PrimaryKeyTableName&& propertyModel.Name != = foreignKeyModel.PrimaryKeyName)) {
        ...
        rowCell =
            new mxCell(cellName, new mxGeometry(0, 0, 90, 26),
        ...
        var columnType = propertyModel.IsPrimaryKey && propertyModel.IsForeignKey ? "PK | FK"
                         : propertyModel.IsPrimaryKey                             ? "PK"
                         : propertyModel.IsForeignKey                             ? "FK"
                                                                                  : "";
    ...
    function ParseMySQLForeignKey(name, currentTableModel)
    {
        ...
        var foreignKeyDestinationModel =
            CreateForeignKey(referencedPropertyName, referencedTableName, foreignKey, currentTableModel.Name, false);
    ...
    function ParseSQLServerForeignKey(name, currentTableModel)
    {
        ...
        if (name.toLowerCase().indexOf("foreign key(") != = -1) {
            var foreignKeySQL =
                name.substring(name.toLowerCase().indexOf("foreign key("), referencesIndex).replace("FOREIGN KEY(", "").replace(")", "");
        } else {
            var foreignKeySQL =
                name.substring(name.toLowerCase().indexOf("foreign key ("), referencesIndex).replace("FOREIGN KEY (", "").replace(")", "");
        }
        ...
        if (referencesIndex != = -1 && alterTableName != = "" && foreignKeySQL != = "" && referencesSQL != = "") {
            ...
            // Create ForeignKey
            var foreignKeyDestinationModel =
                CreateForeignKey(referencedPropertyName, referencedTableName, foreignKey, alterTableName, false);
    ...
    function parseSql(text, type)
    {
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            // Parse Properties
                ...
                var normalProperty = propertyType != = "primary key" && propertyType != = "foreign key" && propertyType != =
                    "constrain primary key" && propertyType != = "constrain foreign key";
                ...
                if (normalProperty) {
                    ...
                    if (MODE_SQLSERVER) {
                        if (name.indexOf(" ASC") != = -1 || name.indexOf(" DESC") != = -1 || name.indexOf(" EXEC") != =
                                -1 || name.indexOf(" WITH") != = -1 || name.indexOf(" ON") != = -1 || name.indexOf(" ALTER") != =
                                    -1 || name.indexOf("/*") != = -1 || name.indexOf(" CONSTRAIN") != = -1 || name.indexOf(" SET") != =
                                        -1 || name.indexOf(" NONCLUSTERED") != = -1 || name.indexOf(" GO") != =
                                            -1 || name.indexOf(" REFERENCES") != = -1) {
                            continue;
                        }
                ...
                if (propertyType === "foreign key" || propertyType === "constrain foreign key") {
                    if (!MODE_SQLSERVER) {
                        ...
                        if (name.indexOf("REFERENCES") === -1) {
                            ...
                            completeRow =
                                "ALTER TABLE [dbo].[" + currentTableModel.Name + "]  WITH CHECK ADD" + " " + name + " " + referencesRow;
                        }
    ...
    function CreateTableUI()
    {
        tableList.forEach(function(tableModel) {
            ...
            // Create Table
            tableCell =
                new mxCell(tableModel.Name, new mxGeometry(dx, 0, maxNameLenght, 26),
    ...
    var resetBtn = mxUtils.button(mxResources.get("reset"), function() { sqlInput.value = ""; });
