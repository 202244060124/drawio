...
Draw.loadPlugin(function(editorUi) {
    ...
    function main()
    {
        ...
        if (name != null && card != null) {
            ...
            {
                editorUi.createFile(name,
                                    templateData
                                        || editorUi.getFileData(/ (\.xml)$ / i.test(name) || name.indexOf(".") < 0,
                                                                / (\.svg)$ / i.test(name), / (\.html)$ / i.test(name)),
                                    null, "trello", null, true, card);
            };

            if (template != null) {
                editorUi.trello.getFile(
                    card + editorUi.trello.SEPARATOR + template, function(file) { doCreateFile(file.getData()); },
