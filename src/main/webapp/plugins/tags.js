...
Draw.loadPlugin(function(editorUi) {
    ...
    editorUi.actions.addAction(
        "hiddenTags...", function() {
            if (editorUi.hiddenTagsWindow == null) {
                ...
                editorUi.hiddenTagsWindow.window.addListener("show", function() { editorUi.fireEvent(new mxEventObject("hiddenTags")); });
                editorUi.hiddenTagsWindow.window.addListener("hide", function() { editorUi.fireEvent(new mxEventObject("hiddenTags")); });
    ...
    var HiddenTagsWindow = function(editorUi, x, y, w, h)
    {
        ...
        function getAllTags()
        {
            return graph.getTagsForCells(graph.model.getDescendants(graph.model.getRoot()));
        }
        ...
        graph.isCellVisible = function(cell)
        {
            return (graphIsCellVisible.apply(this, arguments)
                    && (hiddenTagCount == 0 || !matchTags(graph.getTagsForCell(cell), hiddenTags, hiddenTagCount)));
        };
        ...
        graph.selectionModel.addListener(mxEvent.CHANGE, function(sender, evt) { refreshUi(); });

        graph.model.addListener(mxEvent.CHANGE, function(sender, evt) { refreshUi(); });

        mxEvent.addListener(filterInput, "keyup", function() { updateTagCloud(getAllTags()); });
        ...
        this.window.addListener("show", mxUtils.bind(
                                            this, function() {
                                                this.window.fit();
        ...
        var resizeListener = mxUtils.bind(
            this, function() {
