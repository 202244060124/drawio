...
Draw.loadPlugin(function(ui) {
    ...
    if (ui.editor.isChromelessView()) {
        ...
        model.addListener(
            mxEvent.CHANGE, function(sender, evt) {
                if (tape != null) {
            ...
            "record...", function() {
                if (tape == null) {
                    ...
                    var dlg = new FilenameDialog(
                        ui, 1000, mxResources.get("apply"),
                        function(newValue) {
                            if (newValue != null) {
                                var dlg = new EmbedDialog(ui, "https://www.draw.io/?p=replay&lightbox=1&replay-delay="
                                                                  + parseFloat(newValue) + "&replay-data="
                                                                  + Graph.compress("<recording>" + tmp.join("") + "</recording>"));
        ...
        ui.actions.addAction(
            "replay...", function() {
                var dlg = new TextareaDialog(
                    ui, "Changes [JSON export, compressed edits or <edit>..</edit>]:", "", function(newValue) {
