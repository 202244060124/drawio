...
Draw.loadPlugin(function(ui) {
    ...
    if (!ui.editor.isChromelessView()) {
        ...
        // Adds action
        ui.actions.addAction("extractData...", function() { dataWindow.setVisible(!dataWindow.isVisible()); });
    ...
    function cellClicked(cell)
    {
        ...
        if (cell == null) {
            ...
            if (!ui.editor.isChromelessView()) {
                ...
                mxEvent.addListener(button, "click", function(evt) { extractData(evt); });
    ...
    if (!ui.editor.isChromelessView()) {
        graph.selectionModel.addListener(mxEvent.CHANGE, function(sender, evt) { cellClicked(graph.getSelectionCell()); });

        graph.model.addListener(mxEvent.CHANGE, function(sender, evt) { cellClicked(graph.getSelectionCell()); });
    } else {
        graph.click = function(me)
        {
            // Async required to enable hyperlinks in labels
            window.setTimeout(function() { cellClicked(me.getCell()); }, 0);
        };
    }
});
