...
Draw.loadPlugin(function (ui) {
    ...
    ui.actions.addAction(
        ...
        function() {
            if (graph.isEnabled() && graph.getSelectionCount() == 1) {
                ...
                graph.traverse(
                    cell, true, function(vertex, edge) {
                        if (edge != null) {
                            subtree.push(edge);
                        }
        ...
        try {
            ...
            for (var i = 0; i < cells.length; i++) {
                if (isTreeCell(cells[i])) {
                    graph.traverse(
                        cells[i], true, function(vertex, edge) {
                            if (edge != null) {
                                tmp.push(edge);
                            }
                ...
                for (var i = 0; i < cells.length; i++) {
                    if (isTreeCell(cells[i])) {
                        ...
                        if (newEdges.length == 0 && edges.length > 0) {
                            ...
                            this.addEdge(clone, graph.getDefaultParent(), this.model.getTerminal(edges[0], true), result[i]);
                        }
                    }
                }
            }
    ...
    graph.moveCells = function(cells, dx, dy, clone, target, evt, mapping)
    {
        ...
        try {
            ...
            if (isTreeCell(target)) {
                ...
                for (var i = 0; i < cells.length; i++) {
                    if (isTreeCell(cells[i]) || (graph.model.isEdge(cells[i]) && graph.model.getTerminal(cells[i], true) == null)) {
                        target = null;
                        break;
                    }
                }
    ...
    function addSibling(cell, after)
    {
        ...
        try {
            ...
            if (dir == mxConstants.DIRECTION_SOUTH || dir == mxConstants.DIRECTION_NORTH) {
                clones[1].geometry.x += after ? cell.geometry.width + spacing : -clones[1].geometry.width - spacing;
            } else {
                clones[1].geometry.y += after ? cell.geometry.height + spacing : -clones[1].geometry.height - spacing;
            }
            ...
            if (state != null) {
                ...
                if (dir == mxConstants.DIRECTION_SOUTH || dir == mxConstants.DIRECTION_NORTH) {
                    bbox.x += (after ? cell.geometry.width + spacing : -clones[1].geometry.width - spacing) * s;
                } else {
                    bbox.y += (after ? cell.geometry.height + spacing : -clones[1].geometry.height - spacing) * s;
                }
                            ...
                            if (temp != cell && sibling != null) {
                                if ((hor && after != sibling.getCenterX() < state.getCenterX())
                                    ...
                                    if (mxUtils.intersects(bbox, sibling)) {
                                        dx = spacing
                                             + Math.max(dx, (Math.min(bbox.x + bbox.width, sibling.x + sibling.width)
                                                             - Math.max(bbox.x, sibling.x))
                                                                / s);
                                        dy = spacing
                                             + Math.max(dy, (Math.min(bbox.y + bbox.height, sibling.y + sibling.height)
                                                             - Math.max(bbox.y, sibling.y))
                                                                / s);
                                    }
                                }
                            }
                        ...
                        if (dir == getTreeDirection(temp)) {
                            ...
                            if (temp != cell && sibling != null) {
                                if ((hor && after != sibling.getCenterX() < state.getCenterX())
                                    ...
                                    graph.traverse(
                                        sibling.cell, true, function(vertex, edge) {
                                            if (edge != null) {
                                                subtree.push(edge);
                                            }
    ...
    function addChild(cell)
    {
        ...
        try {
            ...
            if (dir == mxConstants.DIRECTION_SOUTH) {
                clones[1].geometry.x = bbox == null ? cell.geometry.x + (cell.geometry.width - clones[1].geometry.width) / 2
                                                    : (bbox.x + bbox.width) / s - tr.x + spacing;
                ...
                clones[1].geometry.y = bbox == null ? cell.geometry.y + (cell.geometry.height - clones[1].geometry.height) / 2
                                                    : (bbox.y + bbox.height) / s - tr.y + spacing;
            } else {
    ...
    function getOrderedTargets(cell, horizontal, ref)
    {
        ...
        if (state != null && sib != null) {
            for (var i = 0; i < sib.length; i++) {
                ...
                if (temp != null
                    && ((!horizontal && Math.min(temp.x + temp.width, state.x + state.width) >= Math.max(temp.x, state.x))
                        || (horizontal && Math.min(temp.y + temp.height, state.y + state.height) >= Math.max(temp.y, state.y)))) {
                    targets.push(temp);
                }
            }

            targets.sort(function(a, b) { return horizontal ? a.x + a.width - b.x - b.width : a.y + a.height - b.y - b.height; });
        }
    ...
    {
        try {
            if (graph.isEnabled() && !graph.isEditing() && graph.getSelectionCount() == 1 && isTreeCell(graph.getSelectionCell())) {
                var cells = null;

                if (graph.getSelectionCell().getAttribute("treeRoot") != "1") {
                    if (evt.which == 9) {
                        // Tab adds child
                        cells = mxEvent.isShiftDown(evt) ? addParent(graph.getSelectionCell()) : addChild(graph.getSelectionCell());
    ...
    graph.connectVertex = function(source, direction, length, evt, forceClone, ignoreCellAt)
    {
        if (isTreeCell(source) && source.getAttribute("treeRoot") != "1") {
            ...
            if (dir == direction) {
                ...
                return addSibling(source, direction != mxConstants.DIRECTION_NORTH && direction != mxConstants.DIRECTION_WEST);
            }
    ...
    graph.graphHandler.getCells = function(initialCell)
    {
        ...
        for (var i = 0; i < temp.length; i++) {
            if (isTreeCell(temp[i])) {
                ...
                graph.traverse(
                    temp[i], true, function(vertex, edge) {
    ...
    function mxIconSet(state)
    {
        ...
        mxEvent.addGestureListeners(img, mxUtils.bind(
                                             this, function(evt) {
                                                 ...
                                                 graph.graphHandler.bounds =
                                                     graph.graphHandler.graph.getView().getBounds(graph.graphHandler.cells);
...
, mouseMove : function(sender, me)
{
    ...
    if ((graph.isMouseDown && !mxEvent.isTouchEvent(me.getEvent())) || graph.isEditing()
        || (tmp != null && (!graph.getModel().isVertex(tmp.cell) || !isTreeCell(me.getCell())))) {
        tmp = null;
    }
...
, mouseUp : function(sender, me){}, dragEnter : function(evt, state)
{
...
{
    sb.addPalette(
        "trees", "Trees", false, function(content) {
            (function() {
                var cell = new mxCell("Central Idea", new mxGeometry(0, 20, 100, 40),
                                      "ellipse;whiteSpace=wrap;html=1;align=center;" + "collapsible=0;container=1;recursiveResize=0;");
                ...
                var edge = new mxCell("", new mxGeometry(0, 0, 0, 0),
                                      "edgeStyle=entityRelationEdgeStyle;" + "startArrow=none;endArrow=none;segment=10;curved=1;");
            ...
            (function() {
                ...
                var cell2 =
                    new mxCell("Division", new mxGeometry(0, 100, 100, 60),
                               "whiteSpace=wrap;html=1;align=center;verticalAlign=middle;" + "collapsible=0;container=1;recursiveResize=0;");
                cell2.vertex = true;

                var edge = new mxCell("", new mxGeometry(0, 0, 0, 0),
                                      "edgeStyle=elbowEdgeStyle;elbow=vertical;" + "startArrow=none;endArrow=none;rounded=0;");
                edge.geometry.relative = true;
                edge.edge = true;
                ...
                var cell3 =
                    new mxCell("Division", new mxGeometry(140, 100, 100, 60),
                               "whiteSpace=wrap;html=1;align=center;verticalAlign=middle;" + "collapsible=0;container=1;recursiveResize=0;");
                cell3.vertex = true;
            ...
            (function() {
                var cell =
                    new mxCell("Sub Tree", new mxGeometry(0, 0, 100, 60),
                               "whiteSpace=wrap;html=1;align=center;verticalAlign=middle;" + "collapsible=0;container=1;recursiveResize=0;");
                cell.vertex = true;
            ...
            (function() {
                var cell =
                    new mxCell("Sub Section", new mxGeometry(0, 0, 100, 60),
                               "whiteSpace=wrap;html=1;align=center;verticalAlign=middle;" + "collapsible=0;container=1;recursiveResize=0;");
                cell.vertex = true;

                var edge =
                    new mxCell("", new mxGeometry(0, 0, 0, 0),
                ...
                var cell2 =
                    new mxCell("Sub Section", new mxGeometry(120, 0, 100, 60),
                               "whiteSpace=wrap;html=1;align=center;verticalAlign=middle;" + "collapsible=0;container=1;recursiveResize=0;");
                cell2.vertex = true;
