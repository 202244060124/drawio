...
Draw.loadPlugin(function(ui) {
    ...
    ui.getCurrentUser = function()
    {
        if (ncUser == null) {
            ui.remoteInvoke(
                ...
                function(user) {
                    ncUser = user == null ? new DrawioUser(Date.now(), null, "Anonymous") : new DrawioUser(user.uid, null, user.displayName);
                },
                function(){
                ...
                var doSave = mxUtils.bind(
                    this, function() {
                        try {
                            ...
                            ui.remoteInvoke("saveFile",
                                            this.desc.ver > 1 ? [ this.desc.id, this.desc.shareToken, savedData, etag ]
                                                              : [ this.desc.path, savedData, etag ],
                                            null,
                                            mxUtils.bind(
                                                this,
                                                function(resp) {
                                                                    ...
                                                                    .fileConflict(null,
                                                                                  mxUtils.bind(
                                                                                      ...
                                                                                      function() {
                                                                                          ...
                                                                                          EditorUi.debug("EmbedFile.saveFile.conflict", [this],
                                                                                                         "err", err, "delay", delay);
                                                                                      }),
                                                                                  mxUtils.bind(
                                                                                      this, function() {
    ...
    {
        ui.remoteInvoke("loadFile", this.desc.ver > 1 ? [ this.desc.id, this.desc.shareToken ] : [this.desc.path], null,
                        mxUtils.bind(
                            this,
                            function(data) {
                                var xml = data.xml;
    ...
    {
        origInstallMessageHandler.call(
            this, function(xml, evt) {
                try {
                    // New empty files
                    if (JSON.parse(evt.data).xml == " ")
                        return;
                } catch (e) {
                } // Ignore
