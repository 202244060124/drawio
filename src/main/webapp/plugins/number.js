...
graph.view.validateCellState = function(cell, recurse)
{
    ...
    if (recurse && state != null && graph.model.isVertex(state.cell) && mxUtils.getValue(state.style, "numbered", 1) == 1) {
        this.numberCounter++;
...
graph.view.redrawNumberShape = function(state)
{
    ...
    var value = '<div style="padding:2px;border:1px solid gray;background:yellow;border-radius:2px;">' + this.numberCounter + "</div>";

    if (enabled && numbered && graph.model.isVertex(state.cell) && state.shape != null && state.secondLabel == null) {
        state.secondLabel = new mxText(value, new mxRectangle(), mxConstants.ALIGN_LEFT, mxConstants.ALIGN_BOTTOM);
...
// Adds action
var action = ui.actions.addAction(
    "number...", function() {
        enabled = !enabled;
        graph.refresh();
    });
